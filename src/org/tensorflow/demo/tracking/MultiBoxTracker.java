...
public class MultiBoxTracker {
  ...
  private static final int MAX_TRACKED_OBJECTS = 10;
  ...
  private void processResults(
    ...
    if (objectTracker == null) {
      ...
      for (final Pair<Float, Recognition> potential : rectsToTrack) {
        ...
        trackedRecognition.color = getRecognitionColor(trackedRecognition);
  ...
  private void handleDetection(
    ...
    // object.
    if (trackedObjects.size() >= MAX_TRACKED_OBJECTS && removeList.isEmpty()) {
  ...
  private int getRecognitionColor(TrackedRecognition recognition){
    switch(recognition.title){
      ...
      case "Broken Sun":
        return Color.YELLOW;
      case "Confederate Flag":
      case "NSM Flag":
      case "Southern Nationalist":
      case "KKK Loyal White Knights":
      case "KKK Insignia":
      ...
      case "KEK Flag":
        return Color.CYAN;
      case "Vanguard Fasces":
      case "Traditional Worker's Party":
      case "Identity Evropa":
      case "Vanguard":
      case "Right Wing Death Squad":
